...
async function refineComponent(comp: MisalignedComponent, context: RefinementContext): Promise<ComponentCorrectionLog> {
  ...
  try {
    ...
    const judgerInstruction = formatJudgerInstruction(
      comp,
      figmaMetadata as unknown as Record<string, unknown>,
      componentPaths
    );
...
export async function validationLoop(params: ValidationLoopParams): Promise<ValidationLoopResult> {
    ...
    if (workspaceDir && runCommand && buildCommand) {
      ...
      if (!launchResult.success) {
        ...
        return {
          ...
          finalSae: 0,
          totalIterations: 0,
          processedOutput: { iterations: [], finalResult: { success: false, finalMae: -1, finalSae: 0, totalIterations: 0, misalignedCount: 0 } },
  ...
  if (Math.abs(designOffset[0]) >= 1 || Math.abs(designOffset[1]) >= 1) {
    logger.printLog(
      `Normalized Figma coordinates (offset: ${designOffset[0].toFixed(0)}, ${designOffset[1].toFixed(0)} px)`
    );
  }

  // Build unified element registry (single traversal, replaces 3 separate operations)
  const figmaNodeService = new FigmaNodeService(figmaJson.frames || []);
  ...
  const resolvedComponentPaths = workspaceDir
    ? resolveComponentPaths(componentPaths, workspaceDir)
    : componentPaths;
  ...
  for (let iteration = 1; iteration <= config.maxIterations; iteration++) {
    logger.printLog(`\n${'='.repeat(60)}`);
    logger.printLog(`Iteration ${iteration}/${config.maxIterations}`);
      ...
      if (workspaceDir && buildCommand && serverKey) {
        ...
        if (!buildCheck.success) {
          ...
          return {
            ...
            processedOutput: { iterations, finalResult: { success: false, finalMae: currentMae, finalSae: currentSae, totalIterations: iteration, misalignedCount: misaligned.length } },
  ...
  if (!validationPassed) {
    logger.printWarnLog(
      `Max iterations (${config.maxIterations}) reached without satisfying MAE threshold (${config.targetMae}px). Final MAE: ${currentMae.toFixed(2)}px`
    );
  }
